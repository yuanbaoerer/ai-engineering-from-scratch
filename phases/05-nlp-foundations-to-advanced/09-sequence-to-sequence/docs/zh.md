...
**类型：** 构建
**语言：** Python
**前置知识：** 第5阶段 · 第8课（用于文本的 CNN + RNN），第3阶段 · 第11课（PyTorch 入门）
**时间：** 约75分钟
...
序列到序列（seq2seq）架构（Sutskever, Vinyals, Le, 2014）用一个刻意简单的方案解决了这个问题。两个 RNN。一个读取源句子并生成固定大小的上下文向量。另一个读取该向量并逐个 token 生成目标句子。代码与你为第8课编写的相同，只是组合方式不同。
...
**训练：** 在解码器的每一步计算交叉熵损失（Cross-Entropy Loss），在整个序列上求和。通过时间反向传播（Backpropagation Through Time / BPTT）穿过两个网络。
...
`outputs` 的形状为 `[batch, seq_len, hidden_dim]` —— 每个输入位置一个隐藏状态。`hidden` 的形状为 `[1, batch, hidden_dim]` —— 最后一步的输出。第8课说过"对输出进行池化用于分类"。这里我们将最后一个隐藏状态作为上下文向量，忽略每一步的输出。
...
- **强化学习（Reinforcement Learning）微调。** 用指标奖励序列生成器。用于现代 LLM 的 RLHF。
