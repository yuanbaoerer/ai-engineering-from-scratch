...
**词级分词（Word-level tokenization）** 按空格和标点切分。"The cat sat" 变成 ["The", "cat", "sat"]。很简单。但 "tokenization" 呢？"GPT-4o" 呢？或者德语复合词 "Geschwindigkeitsbegrenzung"？词级分词需要一个巨大的词表来覆盖每种语言的每个单词。漏掉一个词，你就会得到 dreaded `[UNK]` 词元——模型在说"我不知道这是什么"。仅英语就有超过一百万种词形。再加上代码、URL、科学记数法和另外 100 种语言，你需要一个无限大的词表。
...
这就是为什么 Llama 3 将词表从 32K 扩大到 128K。为非农文字分配更多词元意味着跨语言的更公平压缩。
...
| 生育率 (Fertility) | "不是分词术语" | 每个单词的平均词元数——衡量跨语言分词器效率的指标（1.0 是完美的，3.0 意味着模型工作量大三倍） |
